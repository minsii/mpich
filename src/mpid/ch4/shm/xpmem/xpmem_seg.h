...
/*
 * XPMEM AVL Tree segment caching contains three important operations:
 *  - MPIDI_XPMEM_segtree_init
 *  - MPIDI_XPMEM_segtree_do_search_and_insert_safe (private)
 *  - MPIDI_XPMEM_segtree_delete_all
 *
 *  MPIDI_XPMEM_segtree_init:
 *       This function initializes the basic attributes of AVL tree
 *       including root node and tree size. It should be called only once
 *       for a AVL tree at the beginning of the program.
 *
 *  MPIDI_XPMEM_segtree_do_search_and_insert_safe:
 *       This function must be threads safe. It combines search and insert
 *       to save one unnecessary top-down traversal when separating these
 *       two. It first performs binary search to find matched segment; if
 *       it does not find a cached segment, then it inserts a new segment
 *       into AVL tree. The time complexity of search and insert process are
 *       O(logN).
 *       Consider search procedure, assume we have an input segment from user,
 *       this function will find a matched segment if the input segment low
 *       address and high address locate between the recorded segment in AVL
 *       tree. If low address of input segment is smaller than recorded segment,
 *       we need to search left branch; or else, we need to search right
 *       branch.
 *       Consider insert procedure, the key we use to decide the order of
 *       segment is the low address of segment. If low address of a new segment
 *       is smaller than the current segment node in AVL tree, it will be
 *       inserted into left branch; or else, it will be inserted into right
 *       branch. After normal insertion, we check the height of each sub tree to
 *       make sure all of sub trees still obey the AVL tree requirement (left
 *       branch and right branch height difference cannot exceed 1). If it
 *       exceeds this limit, we need to adjust the height of tree to make it
 *       balance and satisfy the requirement of AVL tree again.
 *
 *  MPIDI_XPMEM_segtree_delete_all:
 *       This function checks reference count of node and output warning
 *       info if reference count is larger than 0, then it releases all the
 *       segment in the AVL tree.
 */

/* MPIDI_XPMEM_seg_do_release check if the reference count of segment is 0.
 * if 0, it decreases the segment map reference count, detach and release
 * the segment. */
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_seg_do_release
#undef FCNAME
...
/* MPIDI_XPMEM_seg_do_create allocates memory for new segment and initialize the segment
 * based on the input parameters and attached address. */
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_seg_do_create
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
/****************************************/
/* AVL tree insertion related functions */
/****************************************/

#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_avl_do_update_node_info
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_avl_do_update_node_info(MPIDI_XPMEM_seg_t * node)
{
    ...
    node->min = node->left == NULL ? node->min : node->left->min;
    node->max = node->right == NULL ? node->max : node->right->max;
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_avl_do_right_rotation
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_avl_do_left_rotation
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_avl_do_left_rotation(MPIDI_XPMEM_seg_t *
                                                              parent,
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_avl_do_left_right_rotation
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_avl_do_right_left_rotation
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_avl_do_right_left_rotation(MPIDI_XPMEM_seg_t *
                                                                    parent,
                                                                    MPIDI_XPMEM_seg_t * right_child)
{
    int mpi_errno = MPI_SUCCESS;
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_segtree_do_create_node
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_segtree_do_search_and_insert_safe
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
/****************************************/
/* Segment AVL tree related functions   */
/****************************************/

#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_segtree_init
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_segtree_delete_all
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
/******************************************/
/* Segment registration related functions */
/******************************************/

/* Registers a segment for users and return corresponding buffer address
 * to user. It calls AVL tree function MPIDI_XPMEM_segtree_do_tree_search_and_insert
 * to search cached segments and insert a new one if it does not hit the cache. */
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_seg_regist
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
                                                    ...
                                                    MPIDI_XPMEM_seg_t ** seg_ptr, void **vaddr)
{
    ...
    mpi_errno = MPIDI_XPMEM_segtree_do_search_and_insert_safe(&segmap->segcache, seg_low, seg_high,
    ...
    *seg_ptr = seg;
    ...
    XPMEM_DBG_PRINT("register segment %p(refcount %d) for node_rank %d, apid 0x%lx, "
                    "size 0x%lx->0x%lx, seg->low %p->0x%lx, attached_vaddr %p, vaddr %p\n",
                    seg, MPIR_cc_get(seg->refcount), node_rank, (uint64_t) segmap->apid,
                    size, seg_size, remote_vaddr, seg->low, seg->vaddr, *vaddr);
...
/* Decreases reference count of a segment and check its value >=0. */
#undef FUNCNAME
...
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_seg_deregist(MPIDI_XPMEM_seg_t * seg)
{
    ...
    XPMEM_DBG_PRINT("deregister segment %p(refcount %d) vaddr=%p\n",
                    seg, MPIR_cc_get(seg->refcount), seg->vaddr);
