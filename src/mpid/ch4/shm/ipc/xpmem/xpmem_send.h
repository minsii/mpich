...
MPL_STATIC_INLINE_PREFIX int MPIDI_IPC_xpmem_lmt_isend(const void *buf, MPI_Aint count,
                                                       ...
                                                       MPIR_Request ** request)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPC_XPMEM_LMT_ISEND);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPC_XPMEM_LMT_ISEND);
    ...
    *request = sreq;
    ...
    mpi_errno = MPIDI_SHM_do_ctrl_send(rank, comm, MPIDI_SHM_IPC_XPMEM_SEND_LMT_RTS, &ctrl_hdr);
    MPIR_ERR_CHECK(mpi_errno);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPC_XPMEM_LMT_ISEND);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
