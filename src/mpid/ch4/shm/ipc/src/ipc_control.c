...
int MPIDI_IPC_ctrl_send_lmt_recv_fin_cb(MPIDI_SHM_ctrl_hdr_t * ctrl_hdr)
{
    ...
    MPIR_Request *sreq = (MPIR_Request *) ctrl_hdr->ipc_slmt_recv_fin.req_ptr;
    ...
    mpi_errno = MPIDI_IPC_xpmem_send_lmt_recv_fin_cb_hook(ctrl_hdr);
...
int MPIDI_IPC_ctrl_send_lmt_send_fin_cb(MPIDI_SHM_ctrl_hdr_t * ctrl_hdr)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_SEND_FIN_CB);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_SEND_FIN_CB);

    IPC_TRACE("send_lmt_send_fin_cb: complete rreq %p\n", rreq);

#ifdef MPIDI_CH4_SHM_ENABLE_XPMEM
    mpi_errno = MPIDI_IPC_xpmem_send_lmt_send_fin_cb_hook(ctrl_hdr);
    MPIR_ERR_CHECK(mpi_errno);
#endif

    /* send_fin_cb can only be triggered in cooperative copy, so
     * MPIDI_IPC_REQUEST(rreq, counter_ptr) must be set by receiver */
    MPIR_Datatype_release_if_not_builtin(MPIDIG_REQUEST(rreq, datatype));
    MPID_Request_complete(rreq);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_SEND_FIN_CB);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
...
int MPIDI_IPC_ctrl_send_lmt_rts_cb(MPIDI_SHM_ctrl_hdr_t * ctrl_hdr)
{
    ...
    if (rreq) {
        ...
        mpi_errno = MPIDI_IPC_xpmem_unexp_rreq_prep(slmt_rts_hdr->handle.xpmem, rreq);
        MPIR_ERR_CHECK(mpi_errno);
...
int MPIDI_IPC_ctrl_send_lmt_cts_cb(MPIDI_SHM_ctrl_hdr_t * ctrl_hdr)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Request *sreq;
    MPIDI_SHM_ctrl_ipc_send_lmt_cts_t *slmt_cts_hdr = &ctrl_hdr->ipc_slmt_cts;
    MPIDI_IPC_seg_t *seg_ptr = NULL;
    MPIDI_IPC_seg_t *counter_seg_ptr = NULL;
    void *dest_buf = NULL;
    void *src_buf = NULL;
    size_t data_sz ATTRIBUTE((unused));
    MPI_Aint dt_true_lb;
    MPIDI_IPC_cnt_t *counter_ptr = NULL;
    int fin_type, copy_type;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CTS_CB);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CTS_CB);

    /* Sender gets the CTS packet from receiver and need to perform copy */
    sreq = (MPIR_Request *) slmt_cts_hdr->sreq_ptr;

    mpi_errno =
        MPIDI_IPC_seg_regist(slmt_cts_hdr->dest_lrank, slmt_cts_hdr->data_sz,
                             (void *) slmt_cts_hdr->dest_offset, &seg_ptr, &dest_buf,
                             &MPIDI_IPC_global.segmaps[slmt_cts_hdr->dest_lrank].segcache_ubuf);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    if (slmt_cts_hdr->coop_counter_direct_flag) {
        counter_ptr = (MPIDI_IPC_cnt_t *) ((char *)
                                           MPIDI_IPC_global.
                                           coop_counter_direct[slmt_cts_hdr->dest_lrank] +
                                           slmt_cts_hdr->coop_counter_offset);
    } else {
        MPIDI_IPC_seg_regist(slmt_cts_hdr->dest_lrank,
                             sizeof(MPIDI_IPC_cnt_t),
                             (void *) slmt_cts_hdr->coop_counter_offset,
                             &counter_seg_ptr, (void **) &counter_ptr,
                             &MPIDI_IPC_global.segmaps[slmt_cts_hdr->dest_lrank].segcache_cnt);
    }

    /* sender and receiver datatypes are both continuous, perform cooperative copy. */
    mpi_errno =
        MPIDI_IPC_do_lmt_coop_copy(src_buf, slmt_cts_hdr->data_sz, dest_buf,
    ...
    if ((fin_type == MPIDI_IPC_LOCAL_FIN && copy_type == MPIDI_IPC_COPY_ALL) ||
        ...
        if (fin_type == MPIDI_IPC_BOTH_FIN) {
            ...
            if (copy_type == MPIDI_IPC_COPY_ZERO) {
                ack_ctrl_hdr.u.ipc_slmt_cnt_free.coop_counter_direct_flag =
                    slmt_cts_hdr->coop_counter_direct_flag;
                ack_ctrl_hdr.u.ipc_slmt_cnt_free.coop_counter_offset =
                    slmt_cts_hdr->coop_counter_offset;
                ctrl_id = MPIDI_SHM_IPC_SEND_LMT_CNT_FREE;
            } else {
                ack_ctrl_hdr.u.ipc_slmt_send_fin.req_ptr = slmt_cts_hdr->rreq_ptr;
    ...
    mpi_errno = MPIDI_IPC_seg_deregist(seg_ptr);
    MPIR_ERR_CHECK(mpi_errno);

    if (counter_seg_ptr) {
        mpi_errno = MPIDI_IPC_seg_deregist(counter_seg_ptr);
        MPIR_ERR_CHECK(mpi_errno);
    }

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CTS_CB);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
...
int MPIDI_IPC_ctrl_send_lmt_cnt_free_cb(MPIDI_SHM_ctrl_hdr_t * ctrl_hdr)
{
    ...
    MPIDI_IPC_cnt_t *counter_ptr;
    MPIDI_SHM_ctrl_ipc_send_lmt_cnt_free_t *ipc_slmt_cnt_free = &ctrl_hdr->ipc_slmt_cnt_free;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CNT_FREE_CB);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CNT_FREE_CB);

    if (ipc_slmt_cnt_free->coop_counter_direct_flag)
        counter_ptr = (MPIDI_IPC_cnt_t *) ((char *) MPIDI_IPC_cnt_mem_direct +
                                           ipc_slmt_cnt_free->coop_counter_offset);
    else
        counter_ptr = (MPIDI_IPC_cnt_t *) ipc_slmt_cnt_free->coop_counter_offset;
    MPIR_Handle_obj_free(&MPIDI_IPC_cnt_mem, counter_ptr);
    MPIR_cc_decr(&MPIDI_IPC_global.num_pending_cnt, &c);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPC_CTRL_SEND_LMT_CNT_FREE_CB);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
