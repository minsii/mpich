...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_mpi_win_allocate_hook(MPIR_Win * win)
{
     ...
     * more than 1 processes exist on the node, and shm memory has been successfully allocated. */
    if (shm_comm_ptr == NULL || !MPL_proc_mutex_enabled() || !MPIDI_CH4U_WIN(win, mmap_addr))
        goto fn_exit;
