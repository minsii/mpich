...
static void parse_info_perf_preference_str(const char *str, int *perference_ptr)
{
    int perference = 0;
    char *value, *token, *savePtr = NULL;
    ...
    while (token != NULL) {
        if (!strncmp(token, "lat", strlen("lat"))) {
            perference |= (1 << MPIDIG_RMA_LAT_PREFERRED);
        } else if (!strncmp(token, "mr", strlen("mr"))) {
            perference |= (1 << MPIDIG_RMA_MR_PREFERRED);
        }
...
#undef BUFSIZE
    ...
    {   /* Keep buf as a local variable for perf_preference key. */
        char buf[128];
        get_info_accu_ops_str(MPIDIG_WIN(win, info_args).perf_preference, &buf[0], sizeof(buf));
